...
var ErrSettingsChanged = errors.New("the Claude Code status line was changed by another application")
...
func (i Installer) IsConnected() (bool, error) {
	settings, _, err := readSettings(i.Paths.ClaudeSettings())
	if err != nil {
		return false, err
	}
...
func (i Installer) Connect() error {
	...
	if err := copyExecutable(i.Executable, i.Paths.ClaudeHelper()); err != nil {
		return fmt.Errorf("install Claude Code connector: %w", err)
	}

	settings, original, err := readSettings(i.Paths.ClaudeSettings())
	if err != nil {
		return err
	}
	...
	if err := storage.WriteJSON(i.Paths.ClaudeStatusLineBackup(), backup, 0o600); err != nil {
		return fmt.Errorf("back up Claude Code status line: %w", err)
	}
	if len(original) > 0 {
		backupName := fmt.Sprintf("settings.aiquota-backup-%s.json", time.Now().Format("20060102-150405"))
		backupPath := filepath.Join(filepath.Dir(i.Paths.ClaudeSettings()), backupName)
		if err := storage.WriteFileAtomic(backupPath, original, 0o600); err != nil {
			return fmt.Errorf("back up Claude Code settings.json: %w", err)
		}
	}

	settings["statusLine"] = mustJSON(map[string]any{
		"type":            "command",
		"command":         shellQuote(i.Paths.ClaudeHelper()) + " " + BridgeFlag,
		"refreshInterval": 60,
	})
	if err := storage.WriteJSON(i.Paths.ClaudeSettings(), settings, 0o600); err != nil {
		return fmt.Errorf("connect Claude Code status line: %w", err)
	}
	return nil
}
...
func (i Installer) Disconnect() error {
	...
	if err := storage.ReadJSON(i.Paths.ClaudeStatusLineBackup(), &backup); err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			return fmt.Errorf("read Claude Code status line backup: %w", err)
		}
		backup = StatusLineBackup{}
	}
	if backup.HadValue {
...
func readSettings(path string) (map[string]json.RawMessage, []byte, error) {
	...
	if err := json.Unmarshal(data, &settings); err != nil {
		return nil, nil, fmt.Errorf("Claude Code settings.json is invalid: %w", err)
	}
	return settings, data, nil
}
...
func copyExecutable(source, destination string) error {
	...
	temporary, err := os.CreateTemp(filepath.Dir(destination), ".aiquota-bridge-*")
	if err != nil {
		return err
	}
